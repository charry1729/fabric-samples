...
async function readPrivateAsset(assetKey, org, contract) {
  console.log(
    `${GREEN}--> Evaluate Transaction: GetAssetPrivateProperties, - ${assetKey} from organization ${org}${RESET}`
  );
  try {
    ...
    console.log(
      `*** Result: GetAssetPrivateProperties, ${JSON.stringify(asset)}`
    );
  } catch (evalError) {
    console.log(
      `*** Failed evaluateTransaction readPrivateAsset: ${evalError}`
    );
  }
}
...
function checkAsset(org, resultBuffer, ownerOrg) {
  ...
  if (asset) {
    if (asset.ownerOrg === ownerOrg) {
      console.log(
        `*** Result from ${org} - asset ${asset.assetID} owned by ${asset.ownerOrg} DESC:${asset.publicDescription}`
      );
    } else {
      console.log(
        `${RED}*** Failed owner check from ${org} - asset ${asset.assetID} owned by ${asset.ownerOrg} DESC:${asset.publicDescription}${RESET}`
      );
    }
  }
}
...
async function readAssetByBothOrgs(
  ...
  console.log(
    `${GREEN}--> Evaluate Transactions: ReadAsset, - ${assetKey} should be owned by ${ownerOrg}${RESET}`
  );
  let resultBuffer;
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `${GREEN}--> Submit Transaction: CreateAsset, ${assetKey} as Org1 - endorsed by Org1${RESET}`
        );
        ...
        await transaction.submit(
          assetKey,
          `Asset ${assetKey} owned by ${org1} is not for sale`
        );
        console.log(
          `*** Result: committed, asset ${assetKey} is owned by Org1`
        );
      } catch (createError) {
      ...
      try {
        ...
        console.log(
          `${GREEN}--> Submit Transaction: ChangePublicDescription ${assetKey}, as Org1 - endorse by Org1${RESET}`
        );
        transaction = contractOrg1.createTransaction('ChangePublicDescription');
        transaction.setEndorsingOrganizations(org1);
        await transaction.submit(
          assetKey,
          `Asset ${assetKey} owned by ${org1} is for sale`
        );
        console.log(
          `*** Result: committed, asset ${assetKey} is now for sale by Org1`
        );
      } catch (updateError) {
        console.log(
          `${RED}*** Failed: ChangePublicDescription - ${updateError}${RESET}`
        );
      }
      ...
      try {
        ...
        console.log(
          `${GREEN}--> Submit Transaction: ChangePublicDescription ${assetKey}, as Org2 - endorse by Org2${RESET}`
        );
        transaction = contractOrg2.createTransaction('ChangePublicDescription');
        transaction.setEndorsingOrganizations(org2);
        await transaction.submit(
          assetKey,
          `Asset ${assetKey} owned by ${org2} is NOT for sale`
        );
        console.log(
          `${RESET}*** Failed: Org2 is not the owner and this should have failed${RESET}`
        );
      } catch (updateError) {
        console.log(
          `*** Success: ChangePublicDescription has failed endorsememnt by Org2 sent by Org2 - ${updateError}`
        );
      }
      ...
      try {
        ...
        console.log(
          `${GREEN}--> Submit Transaction: ChangePublicDescription ${assetKey}, as Org2 - endorse by Org1${RESET}`
        );
        transaction = contractOrg2.createTransaction('ChangePublicDescription');
        transaction.setEndorsingOrganizations(org1);
        await transaction.submit(
        ...
        console.log(
          `*** Success: ChangePublicDescription has failed endorsement by Org1 sent by Org2 - ${updateError}`
        );
      }
      ...
      try {
        ...
        console.log(
          `${GREEN}--> Submit Transaction: AgreeToSell, ${assetKey} as Org1 - endorsed by Org1${RESET}`
        );
        ...
        console.log(
          `*** Result: committed, Org1 has agreed to sell asset ${assetKey} for 110`
        );
      } catch (sellError) {
      ...
      try {
        ...
        const asset_properties_string = JSON.stringify(asset_properties);
        console.log(
          `${GREEN}--> Evalute: VerifyAssetProperties, ${assetKey} as Org2 - endorsed by Org2${RESET}`
        );
        console.log(`${asset_properties_string}`);
        ...
        if (verifyResultBuffer) {
          ...
          if (verifyResult) {
            console.log(
              `*** Successfully VerifyAssetProperties, private information about asset ${assetKey} has been verified by Org2`
            );
          } else {
            console.log(
              `*** Failed: VerifyAssetProperties, private information about asset ${assetKey} has not been verified by Org2`
            );
          }
        } else {
          console.log(
        ...
        console.log(
          `${RED}*** Failed: VerifyAssetProperties - ${verifyError}${RESET}`
        );
      }
      ...
      try {
        ...
        const asset_price_string = JSON.stringify(asset_price);
        console.log(
          `${GREEN}--> Submit Transaction: AgreeToBuy, ${assetKey} as Org2 - endorsed by Org2${RESET}`
        );
        ...
        await transaction.submit(assetKey);
        console.log(
          `*** Result: committed, Org2 has agreed to buy asset ${assetKey} for 100`
        );
      } catch (buyError) {
      ...
      try {
        ...
        console.log(
          `${GREEN}--> Submit Transaction: TransferAsset, ${assetKey} as Org1 - endorsed by Org1${RESET}`
        );
        console.log(`${asset_properties_string}`);
        ...
        console.log(
          `${RED}*** Failed: committed, TransferAsset should have failed for asset ${assetKey}${RESET}`
        );
      } catch (transferError) {
      ...
      //   await transaction.submit(assetKey);
      //   console.log(
      //     `*** Result: committed, Org1 has agreed to sell asset ${assetKey} for 100`
      //   );
      // } catch (sellError) {
      //   console.log(`${RED}*** Failed: AgreeToSell - ${sellError}${RESET}`);
      // }
      ...
      try {
        ...
        console.log(
          `${GREEN}--> Submit Transaction: TransferAsset, ${assetKey} as Org2 - endorsed by Org1${RESET}`
        );
        console.log(`${asset_properties_string}`);
        ...
        await transaction.submit(assetKey, org2);
        console.log(
          `${RED}*** FAILED: committed, TransferAsset - Org2 now owns the asset ${assetKey}${RESET}`
        );
      } catch (transferError) {
      ...
      try {
        ...
        await transaction.submit(assetKey, org2);
        console.log(
          `*** Results: committed, TransferAsset - Org2 now owns the asset ${assetKey}`
        );
      } catch (transferError) {
        console.log(
          `${RED}*** Failed: TransferAsset - ${transferError}${RESET}`
        );
      }
      ...
      try {
        ...
        console.log(
          '*** Results: committed - Org2 is now the owner and asset is not for sale'
        );
      } catch (updateError) {
        console.log(
          `${RED}*** Failed: ChangePublicDescription has failed by Org2 - ${updateError}${RESET}`
        );
      }
